...
int main() {
	...
	do {
		...
		do {
			t_gen[counter] = cpucycles();
			ret_val = crypto_kem_keypair(pk, sk);
			t_gen[counter + 1] = cpucycles();
		} while (ret_val != 0);

		printf("Key pair generation: %lld", t_gen[counter + 1] - t_gen[counter]);
		printf("\n");
		...
		// Encapsulation
		t_enc[counter] = cpucycles();
		ret_val = crypto_kem_enc(ct, ss, pk);
		t_enc[counter + 1] = cpucycles();

		printf("Encapsulation: %lld", t_enc[counter + 1] - t_enc[counter]);
		printf("\n");
		...
		// Decapsulation
		t_dec[counter] = cpucycles();
		ret_val = crypto_kem_dec(ss1, ct, sk);
		t_dec[counter + 1] = cpucycles();

		printf("Decapsulation: %lld", t_dec[counter + 1] - t_dec[counter]);
		printf("\n");
