...
int syst_mat(binmat_t H) {
	...
	for (i = 0; i < k; i++) {
		...
		if (H.coeff[i][i + n - k] == 0) { //We're looking for a non-zero pivot
			test = 1;
			for (j = 0; j < n; j++) {
				if (H.coeff[i][j]) {
					break;
		...
		if (test == 1) { // We switches the columns J and i + n = k
			test = 0;
			for (l = 0; l < k; l++) {
			...
			if (H.coeff[l][i + n - k]) {
				...
				for (j = 0; j < n; j++) {
					H.coeff[l][j] = H.coeff[l][j]
							^ (gf_mult_fast(piv_align, H.coeff[i][j]));
				}
			}
		}
	}
...
int matrix_inverse(binmat_t A, binmat_t S) {
	...
	for (i = 0; i < k; i++) {
		j = i;
		if (H.coeff[i][i] == 0) { //We're looking for a non-zero pivot
			test = 1;
			for (j = i + 1; j < n; j++) {
				if (H.coeff[j][i]) {
					//r++;
		...
		if (test == 1) { // We switches line j and i
			test = 0;
			for (l = 0; l < k; l++) {
				temp = H.coeff[i][l];
...
void display_matrix(binmat_t H) {
	for (int row = 0; row < H.rown; row++) {
		for (int columns = 0; columns < H.coln; columns++) {
			printf("%d   ", H.coeff[row][columns]);
		}
		printf("\n");
	}
}
