...
binmat_t
produit_matrix (binmat_t A, binmat_t B)
{
  binmat_t res;
  if (A.coln != B.rown)
    {
...
binmat_t
produit_matrix_subfield (binmat_t A, binmat_t B)
{
  binmat_t Res;
  if (A.coln != B.rown)
    {
...
void
mat_rowxor (binmat_t A, int i, int j)
{
  int k;
  for (k = 0; k < A.coln; k++)
    {
...
void
mat_rowxor_with_another (binmat_t A, int i, gf * Line)
{
  int k;
  for (k = 0; k < A.coln; k++)
    {
...
void
mat_swapcol (binmat_t A, int i, int j)
{
  int k;
  gf tmp[A.rown];
  for (k = 0; k < A.rown; k++)
...
void
mat_swaprow (binmat_t A, int i, int j)
{
  int k;
  gf tmp[A.coln];
  for (k = 0; k < A.coln; k++)
...
void
mat_line_mult_by_gf (binmat_t A, gf a, int i)
{
  int j;
  for (j = 0; j < A.coln; j++)
    {
...
gf*
mat_line_mult_with_return (binmat_t A, gf a, int i)
{
  int j;
  gf * Line;
...
int
equals_matrxi (binmat_t H, binmat_t S)
{
  int i, j;
  for (i = 0; i < H.rown; i++)
    {
	  ...
	  if (H.coeff[l][i + n - k])
	      ...
	      for (j = 0; j < n; j++)
		{
		  H.coeff[l][j] = H.coeff[l][j]
		      ^ (gf_Mult_subfield(piv_align, H.coeff[i][j]));
		}
	    }
...
void
G_mat (binmat_t G, binmat_t H_syst)
{
  int i, j;
  binmat_t H;
  H = mat_copy (H_syst);
...
void
affiche_vecteur (gf * v, int taille)
{
  printf ("\n~~ Vector display ~~\n");
  int i = 0;
  for (i = 0; i < taille; i++)
...
gf*
produit_matrix_vector_subfield (binmat_t A, gf* v)
{
  int i, k;
...
gf*
produit_vector_matrix_subfield (gf* v, binmat_t A)
{
  int i, k;
...
gf*
produit_vector_matrix (gf* v, binmat_t A)
{
  int i, k;
  gf* Res = (gf*) calloc (A.coln, sizeof(gf));
  for (i = 0; i < A.coln; i++)
...
gf*
produit_vector_matrix_Sf (gf* v, binmat_t A)
{
  int i, k;
  gf* Res = (gf*) calloc (A.coln, sizeof(gf));
  for (i = 0; i < A.coln; i++)
...
gf*
produit_matrix_vector (binmat_t A, gf* v)
{
  int i, k;
  gf* Res = (gf*) calloc (A.rown, sizeof(gf));
  for (i = 0; i < A.rown; i++)
...
void
Permut_vecteur (int * P, gf* c, int taille)
{
  int i = 0;
...
int
inverse_matrice (binmat_t A, binmat_t S)
{
...
void
secret_matrix (binmat_t H, gf * u, gf * v, gf * z)
{
  binmat_t H_fin;
...
void
quasi_dyadic_bloc_mat (int s, binmat_t M, gf * sig, int ind_col, int ind_rown)
{
  int i, j;
